...
def add_to_cart(product_id, customer_name, customer_mobile, quantity):
    conn = create_connection()
    cursor = conn.cursor(dictionary=True)
...
def bill():
    ...
    if request.method == 'POST':
        try:
            ...
            add_to_cart(product_id, customer_name, customer_mobile, quantity)
    ...
    template = 'bill.html' if session.get('role') == 'admin' else 'employee_bill.html'
    return render_template(template, products=products, cart=cart, grand_total=grand_total, error=error)
